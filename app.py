...
class FaceRecognitionApp:
    ...
    def register_face_gui(self):
        ...
        id_label = tk.Label(register_window, text="ID Number:")
        id_label.pack()
        ...
        browse_btn = tk.Button(register_window, text="Select Image", command=lambda: self.register_face(register_window, id_entry.get(), name_entry.get(), age_entry.get(), gender_entry.get()))
        browse_btn.pack()

    def register_face(self, register_window, id_number, name, age, gender):
        if not id_number or not name or not age or not gender:
        ...
        if file_path:
            ...
            if face_encoding:
                ...
                details = {'id': id_number, 'name': name, 'age': age, 'gender': gender, 'image_path': None}

                # Save the image to the "register" folder
                image_filename = f"{id_number}_{name}_{age}_{gender}.png"
                ...
                tk.messagebox.showerror("Error", "No face found in the selected image.")
    ...
    def recognize_face(self, image):
        ...
        if not face_encodings:
            tk.messagebox.showinfo("Info", "No face found in the selected image.")
            return
    ...
    def display_recognized_details(self, details_list):
        ...
        if details_list:
            for details in details_list:
                details_label = tk.Label(details_window, text=f"ID: {details['id']}\nName: {details['name']}\nAge: {details['age']}\nGender: {details['gender']}")
                details_label.pack()
    ...
    def display_circular_image(self, window, image_path):
        try:
            ...
            tk.messagebox.showerror("Error", f"Error displaying circular image: {e}")
    ...
    def save_registered_faces(self):
        try:
            ...
            tk.messagebox.showerror("Error", f"Error saving registered faces: {e}")
    ...
    def load_registered_faces(self):
        try:
            ...
            tk.messagebox.showerror("Error", f"Error loading registered faces: {e}")
